...
] = """Cash earnings added or removed at each timestamp.
...
] = """Simulation start for each column.
...
] = """Simulation end for each column.
...
] = """Target shape of the simulation data.
...
] = """Time index in integer (nanosecond) format.
...
] = """Tracks valuation price only if it is known.
...
] = """Update group value after each filled order.
...
] = """Latest position for each column.
...
] = """Latest debt per column from leverage or shorting.
...
] = """Latest position record for each column.
...
] = """Sequence of call indices within the current segment.
...
] = """Current call index from `SegmentContext.call_seq_now`.
...
__pdoc__["OrderContext.free_cash_now"] = "`SimulationContext.last_free_cash` for the current column or group."
...
__pdoc__["PostOrderContext.position_before"] = "`OrderContext.position_now` value before order execution."
__pdoc__["PostOrderContext.debt_before"] = "`OrderContext.debt_now` value before order execution."
__pdoc__["PostOrderContext.locked_cash_before"] = "`OrderContext.locked_cash_now` value before order execution."
__pdoc__["PostOrderContext.free_cash_before"] = "`OrderContext.free_cash_now` value before order execution."
__pdoc__["PostOrderContext.val_price_before"] = "`OrderContext.val_price_now` value before order execution."
...
__pdoc__["PostOrderContext.position_now"] = "`OrderContext.position_now` value after order execution."
__pdoc__["PostOrderContext.debt_now"] = "`OrderContext.debt_now` value after order execution."
__pdoc__["PostOrderContext.locked_cash_now"] = "`OrderContext.locked_cash_now` value after order execution."
__pdoc__["PostOrderContext.free_cash_now"] = "`OrderContext.free_cash_now` value after order execution."
__pdoc__[
...
__pdoc__[
    "Order.fees"
] = """Fees as a percentage of the order value.
...
__pdoc__[
    "Order.slippage"
] = """Slippage expressed as a percentage of `Order.price`.
...
__pdoc__[
    "Order.allow_partial"
] = """Indicates whether partial filling of the order is permitted.
...
__pdoc__[
    "Order.log"
] = """Determines whether the order should be logged by creating a log record.
...
__pdoc__[
    "order_dt"
] = f"""NumPy dtype for order records.
...
__pdoc__[
    "fs_order_dt"
] = f"""NumPy dtype for order records generated from signals.
...
__pdoc__[
    "trade_dt"
] = f"""NumPy dtype for trade records.
...
__pdoc__[
    "alloc_range_dt"
] = f"""NumPy dtype for allocation range records.
...
__pdoc__[
    "alloc_point_dt"
] = f"""NumPy dtype for allocation point records.
...
__pdoc__[
    "limit_info_dt"
] = f"""NumPy dtype for limit information records.
...
__pdoc__[
    "sl_info_dt"
] = f"""NumPy dtype for SL information records.
...
__pdoc__[
    "tsl_info_dt"
] = f"""NumPy dtype for TSL information records.
...
__pdoc__[
    "tp_info_dt"
] = f"""NumPy dtype for TP information records.
...
__pdoc__[
    "time_info_dt"
] = f"""NumPy dtype for time information records.
